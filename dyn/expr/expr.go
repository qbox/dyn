...
func init() {
	log.Info("this package is deprecated, please use `github.com/qiniu/dyn/text.v1`")
}
...
func GetAsString(data interface{}, key string, ft int) (val string, err error) {
...
func decodeVar(b []byte, exprvar string, pos int, data interface{}, ft, instring int) ([]byte, int, error) {
	...
	if pos+2 < len(exprvar) {
		...
		case '(', '{':
			...
			if pos2 >= 0 {
				key2 := exprleft[:pos2]
				val2, err2 := GetAsString(data, key2, ft)
				if err2 != nil {
...
func Exec(exprvar string, data interface{}, ft int) (v string, err error) {
	...
	for pos < end {
		...
		case '$':
			...
			b, pos, err = decodeVar(b, exprvar, pos, data, ft, instring)
			if err != nil {
				return
			}
			start, size = pos, 0
